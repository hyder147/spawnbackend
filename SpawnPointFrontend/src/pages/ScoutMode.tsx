        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '2rem 1.5rem' }}>
            ...
            {loading && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-muted)', padding: '2rem 0', justifyContent: 'center' }}>
                    <Loader size={16} className="spin" /> Loading talent board...
                </div>
            )}
            {!loading && error && (
                        ...
                        return (
                            <div key={p.id} style={{
                                ...
                                <div style={{ display: 'flex', gap: '0.9rem', fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
                                    ...
                                    <span style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}><Users size={12} /> {p.squadsLed} squads</span>
                                </div>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.2rem' }}>
                                    ...
                                    {p.available ? (
                                        <button onClick={() => setOfferTarget(p)} className="btn-gradient" style={{ fontSize: '0.72rem', padding: '0.45rem 0.85rem', gap: '0.35rem' }}>
                ...
                <div style={{
                    ...
                    <div onClick={e => e.stopPropagation()} style={{
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                            ...
                            <button onClick={() => { setOfferTarget(null); setOfferNote(''); }} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer' }}><X size={18} /></button>
                        </div>
                        <p style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', lineHeight: 1.5 }}>
                            This sends a direct interview/internship offer using their verified {roleMeta[offerTarget.roleTrack].label} track record — signal score {offerTarget.signalScore}/100.
                        </p>
                        <textarea
                            value={offerNote}
                        ...
                        />
                        <button disabled={sending} onClick={() => sendOffer(offerTarget)} className="btn-gradient" style={{ width: '100%', marginTop: '1rem', justifyContent: 'center', gap: '0.4rem', opacity: sending ? 0.7 : 1 }}>
                            {sending ? <Loader size={14} className="spin" /> : <Send size={14} />} {sending ? 'Sending...' : 'Send Offer'}
                        </button>
                    </div>
                </div>
            )}
