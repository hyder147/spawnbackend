...
const CardTypeBlock: React.FC<{
    info: typeof CARD_TYPES[number]; order: CardOrder | null; pricing: { priceUsd: number } | null;
                    ...
                    {(!order || order.status === 'Rejected') && (
                        <>
                            ...
                            <button className="btn-gradient" onClick={startCheckout} disabled={busy}>
                                ...
                                {busy ? 'Redirecting to checkout...' : `Get My Card — $${pricing?.priceUsd ?? 20}`}
                            </button>
                        </>
                    )}
...
const Cards: React.FC = () => {
    ...
    const [pricing, setPricing] = useState<{ priceUsd: number } | null>(null);
    ...
    const load = useCallback(async () => {
        ...
        try {
            const [o, p] = await Promise.all([
                ...
                api.get<{ priceUsd: number }>('/cards/pricing'),
            ]);
            setOrders(o || []);
            setPricing(p);
